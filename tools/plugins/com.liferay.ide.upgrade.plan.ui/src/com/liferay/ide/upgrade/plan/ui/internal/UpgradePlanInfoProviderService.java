...
public class UpgradePlanInfoProviderService implements UpgradeInfoProvider {
		...
		if (CoreUtil.isNotNullOrEmpty(url)) {
			try {
				...
				if (upgradePlanOutline.isOffline()) {
					detail = _renderArticleMainContent(url, upgradePlanOutline);
				}
				else {
	...
	private File _getEntryFile(String url, String splitor, File location) {
		...
		if (listFiles[0].isFile()) {
			...
			return _getEntryFile(retainedUrls, splitor, listFiles[0]);
		}
	}
