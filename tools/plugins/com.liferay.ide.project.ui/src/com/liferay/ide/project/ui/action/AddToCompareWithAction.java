 ...
 * @author Ethan Sun
 */
public class AddToCompareWithAction implements IObjectActionDelegate {
					...
					try (ZipFile zipFile = new ZipFile(sourceFile)) {
						...
						while (enumeration.hasMoreElements()) {
							...
							if (!entry.isDirectory() && relativePath.contains(entryCanonicalName)) {
								String[] entryNameArr = entryCanonicalName.split("\\.");

								int index = entryNameArr[0].lastIndexOf("/") + 1;

								String entryName = entryCanonicalName.substring(index);
								...
								FileUtils.copyInputStreamToFile(entryInput, sourceEntryFile);
